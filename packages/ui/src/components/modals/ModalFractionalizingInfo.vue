    ...
    <div class="sm:flex sm:items-start">
      ...
      <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
        ...
        <div class="mt-2">
          ...
          <p class="text-sm text-black mb-2">
            The fractionalization process will be over in
            {{ timeLeftToRedeem }}. Please come back later to redeem your $WPX.
          </p>
        </div>
      </div>
    </div>
...
export default {
  setup() {
    ...
    const timeLeftToRedeem = ref(
      formatDistanceToNow(GAME_ENDS_TIMESTAMP + TIME_TO_REDEEM_MILLISECONDS)
    )

    const interval = setInterval(() => {
      timeLeftToRedeem.value = formatDistanceToNow(
        GAME_ENDS_TIMESTAMP + TIME_TO_REDEEM_MILLISECONDS
      )
    }, 10000)
    ...
    return { paintedPixels, timeLeftToRedeem }
  },
}
</script>
